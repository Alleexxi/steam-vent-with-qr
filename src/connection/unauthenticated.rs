...
use crate::auth::{
    AuthConfirmationHandler, GuardDataStore, QrChallengeHandler, begin_password_auth,
    begin_qr_auth,
};
...
impl UnAuthenticatedConnection {
    ...
    pub async fn login<H: AuthConfirmationHandler, G: GuardDataStore>(
        ...
        let steam_id = SteamID::from_steam64(
            begin.steam_id().ok_or(LoginError::InvalidSteamId)?,
        )
    ...
    pub async fn access(
        ...
        let access_token_payload = decode_access_token(access_token)?;
    ...
    /// Start a client session with this connection using a QR code.
    ///
    /// The provided handler is invoked with the challenge URL — render it as
    /// a QR code and have the user scan it with the Steam mobile app.
    /// Steam may rotate the URL during the session; the handler is invoked
    /// again each time it does.
    pub async fn login_qr<H: QrChallengeHandler>(
        self,
        qr_handler: H,
    ) -> Result<Connection, ConnectionError> {
        let mut raw = self.0;
        let mut handler = qr_handler;

        let begin = begin_qr_auth(&raw).await?;
        let initial_url = begin
            .challenge_url()
            .expect("QR auth always carries a challenge URL")
            .to_owned();
        handler.handle_challenge_url(&initial_url).await;

        let qr_tokens = begin.poll().wait_for_qr_tokens(&raw, &mut handler).await?;

        // Decode the refresh token (not the access token): in Steam's JWTs,
        // only the refresh token has `iss: "steam"`. The access token's
        // issuer is a per-session derivation identifier.
        let refresh_payload = decode_access_token(qr_tokens.refresh_token.as_ref())?;
        let steam_id = SteamID::from_steam64(
            refresh_payload
                .sub
                .parse()
        ...
        raw.session = login(
            &mut raw,
            &qr_tokens.account_name,
            steam_id,
            // same quirk as the password path: send the refresh token in the access_token slot
            qr_tokens.refresh_token.as_ref(),
        )
        .await?;
        raw.setup_heartbeat();
        Ok(Connection::new(raw))
...
fn decode_access_token(access_token: &str) -> Result<AccessToken, ConnectionError> {
    let payload = access_token
        .split('.')
        .nth(1)
        .ok_or(ConnectionError::AccessToken(AccessTokenError::Malformed))?;
    let json = BASE64_URL_SAFE_NO_PAD
        .decode(payload)
        .map_err(|e| ConnectionError::AccessToken(AccessTokenError::Base64(e)))?;
    let parsed: AccessToken = serde_json::from_slice(&json)
        .map_err(|e| ConnectionError::AccessToken(AccessTokenError::Json(e)))?;
    if parsed.iss != "steam" {
