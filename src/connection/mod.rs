...
use crate::auth::{AuthConfirmationHandler, GuardDataStore, QrChallengeHandler};
use crate::message::{
...
impl Connection {
    ...
    /// Start a client session on a new connection using a QR code.
    ///
    /// The handler is invoked with the challenge URL; render it as a QR code
    /// and have the user scan it with the Steam mobile app.
    ...
    pub fn account_name(&self) -> Option<&str> {
        self.session().account_name.as_deref()
    }
