...
pub(crate) async fn begin_qr_auth(
    connection: &RawConnection,
) -> Result<StartedAuth, ConnectionError> {
    info!("starting QR login");
    let req = CAuthentication_BeginAuthSessionViaQR_Request {
        device_friendly_name: Some("DESKTOP-VENT".into()),
        platform_type: Some(EnumOrUnknown::new(
            EAuthTokenPlatformType::k_EAuthTokenPlatformType_SteamClient,
        )),
        ...
        ..CAuthentication_BeginAuthSessionViaQR_Request::default()
    };
    let res = service_method_un_authenticated(connection, req).await?;
    Ok(StartedAuth::Qr(res))
}
...
impl StartedAuth {
    fn raw_confirmations(&self) -> &[CAuthentication_AllowedConfirmation] {
        match self {
            ...
            StartedAuth::Qr(res) => res.allowed_confirmations.as_slice(),
        }
    }
    ...
    fn client_id(&self) -> u64 {
        match self {
            ...
            StartedAuth::Qr(res) => res.client_id(),
        }
    }

    pub fn steam_id(&self) -> Option<u64> {
        match self {
            StartedAuth::Credentials(res) => Some(res.steamid()),
            StartedAuth::Qr(_) => None,
        }
    }

    pub fn challenge_url(&self) -> Option<&str> {
        match self {
            StartedAuth::Credentials(_) => None,
            StartedAuth::Qr(res) => Some(res.challenge_url()),
        }
    }
    ...
    fn request_id(&self) -> Vec<u8> {
        match self {
            ...
            StartedAuth::Qr(res) => res.request_id().into(),
        }
    }
    ...
    fn interval(&self) -> f32 {
        match self {
            ...
            StartedAuth::Qr(res) => res.interval(),
        }
    }
        ...
        match confirmation {
            ConfirmationAction::GuardToken(token, ty) => {
                let req = CAuthentication_UpdateAuthSessionWithSteamGuardCode_Request {
                    ...
                    steamid: Some(self.steam_id().unwrap_or_default()),
                    code: Some(token.0),
...
impl PendingAuth {
    ...
    pub(crate) async fn wait_for_qr_tokens<H: QrChallengeHandler>(
        mut self,
        connection: &RawConnection,
        handler: &mut H,
    ) -> Result<QrTokens, ConnectionError> {
        ...
        loop {
            ...
            if response.has_access_token() {
                if !response.has_account_name() {
                    return Err(LoginError::MalformedQrResponse.into());
                }
                return Ok(QrTokens {
                    access_token: Token(response.take_access_token()),
                    refresh_token: Token(response.take_refresh_token()),
                    account_name: response.take_account_name(),
                    new_guard_data: response.new_guard_data,
                });
            }
            ...
            if response.has_new_challenge_url() {
                let new_url = response.take_new_challenge_url();
                if last_url.as_deref() != Some(new_url.as_str()) {
                    handler.handle_challenge_url(&new_url).await;
                    last_url = Some(new_url);
                }
            }

            if response.had_remote_interaction() && !interacted {
                interacted = true;
                handler.handle_remote_interaction().await;
            }
        }
    }
}
